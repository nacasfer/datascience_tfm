...
variantes_DF=pd.read_csv(path , sep='\t', 
                             ...
                             dtype={'causal': 'bool',
                                    ...
                                    'FATHMM_score':'float','genotype':'str',
...
# Filtrado de variantes que no interesan (Annadimos de nuevo variantes causales, por si se descartaron con el filtro y quitamos duplicados)

print('Filtering Non-causal variants')
variantes_casuales=variantes_DF.copy()
variantes_casuales=variantes_casuales[variantes_casuales.causal == True]

print('\nNon causal variants before drop dups: %d \nCausal variants before drop dups: %d'% (variantes_DF.causal.value_counts()[0],variantes_DF.causal.value_counts()[1]))

variantes_DF=flt.filtradoVariantes(variantes_DF)
variantes_DF=pd.concat(variantes_DF,variantes_casuales,ignore_index=True)
variantes_DF = variantes_DF.drop_duplicates()
