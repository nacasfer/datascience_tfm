...
def formateo_allele(row,i,df):
    ''' Funcion para transformar columna genotipe de caracteres en el numero de alelos alternativos '''
    
    if row == 'nan':
        return float('NaN') 
    elif row == '':
        return float('NaN')
    elif row ==  str(df.Ref[i]) +'/'+ str(df.Alt_Annovar[i]) :
        return 1.
    elif row ==  str(df.Ref[i]) +'/'+ str(df.Alt_IR[i]):
        return 1.    
    elif row ==  str(df.Alt_Annovar[i]) +'/'+ str(df.Alt_Annovar[i]):
        return 2.    
    elif row ==  str(df.Alt_IR[i]) +'/'+ str(df.Alt_IR[i]):
        return 2.    
    elif row ==  str(df.Ref[i]) +'/'+ str(df.Ref[i]):
        return 0.    
...
'nonsense':5.,'stoploss':5.,}
    ...
    variantes_DF['PROVEAN_score']=(variantes_DF['PROVEAN_score']+14) / 28
    variantes_DF['CADD_phred']=(variantes_DF['CADD_phred']) / 99
